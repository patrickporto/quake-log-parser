...
class Game:
    ...
    def add_kill(self, killer, victim):
        if killer == WORLD_ID:
            self.kills[victim] = self.kills.get(victim, 0) - 1 if self.kills.get(victim, 0) > 0 else 0
        else:
            self.kills[killer] = self.kills.get(killer, 0) + 1

    def to_dict(self):
        players = set(self.players.values())
        kills = {self.players[player_id]: player_score for player_id, player_score in self.kills.items()}
        return {
            'players': players,
            'kills': kills
        }
        ...
        for (log_record,) in result.fetchall():
            ...
            match tokens:
                ...
                case [_, Token(value='ClientConnect', entity=Entity.COMMAND), *rest]:
                    arguments = [token for token in rest if token.entity == Entity.COMMAND_ARGUMENT]
                    player_id = arguments[0].value
                    games[current_game].add_player(player_id)
                case [_, Token(value='ClientUserinfoChanged', entity=Entity.COMMAND), *rest]:
                    arguments = [token for token in rest if token.entity == Entity.COMMAND_ARGUMENT]
                    player_id = arguments[0].value
